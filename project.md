...
| **รวมเกณฑ์สัญญาอนุญาตเป็นจุดเดียว + ตะกร้า/หน้าจ่ายเงินตามดีไซน์เนอร์** | ✅ โค้ดเสร็จ (1 ส.ค. 2569 — ไม่แตะ DB) · **`designerLicensePdf()` / `designerLicensePdfMap()` ใน `src/lib/license.ts` = จุดตัดสินใจจุดเดียว** เลิกคัดลอกเงื่อนไข 4 ที่ · **`src/components/LicenseLink.tsx`** เลือกเองว่าจะเป็นปุ่มเปิด `PdfLightbox` หรือ `<Link>` ไปฉบับกลาง และ**ถือ state lightbox ไว้ในตัวเอง** ทั้ง 4 หน้าจึงเลิกถือ `pdfOpen` + `<PdfLightbox>` ของตัวเอง · **ตะกร้าเลิกชี้ `/agreement` ตายตัว** — ยิง config รวมด้วย `.in()` จาก `owner_id` ที่ `CartFont` มีอยู่แล้ว แล้วขึ้นบรรทัด "ฟอนต์ของ &lt;ชื่อร้าน&gt; ใช้สัญญาอนุญาตของผู้ออกแบบ" ต่อร้าน · **ทุกร้านใช้ฉบับกลาง = ข้อความเดิมเป๊ะ** (ยืนยันบนเว็บจริงแล้ว) · **`custom_text` บนหน้าจ่ายเงิน Stripe ต่อลิงก์รายร้านด้วย** เพราะจุดนั้นคือที่ที่การยอมรับมีผลจริง (`session.consent`) ชี้แต่ฉบับกลางแล้วหลักฐานจะไม่ตรงกับสัญญาที่บังคับใช้ · cap 5 ร้าน / 1,000 ตัวอักษร (เพดานจริง 1,200) **เกินแล้วถอยไปใช้ข้อความเดิม ห้ามให้สร้าง session ไม่ได้** · อ่าน config ไม่สำเร็จ = ใช้ข้อความเดิม ไม่ล้ม checkout · **⚠️ เส้นทาง PDF ยังไม่ได้ทดสอบสด — DB ยังไม่มีดีไซน์เนอร์รายไหนตั้ง `use_default=false`** |
