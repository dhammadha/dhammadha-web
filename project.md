...
2. **Stripe ขั้นที่ 2 — ทดสอบบน production ด้วย test key** (ขั้นที่ 1 ที่เครื่องผ่านแล้ว)
   ...
   · **🟡 เจอระหว่างทดสอบ ยังไม่แก้: อีเมล delivery ขึ้น "เรียน คุณ " ห้อยลอย ๆ เมื่อไม่มีชื่อ**
     (`email-service.ts:343` → `order.customer_name ?? ""`) · **PromptPay ไม่เก็บชื่อผู้จ่าย**
     (ยืนยัน: `OR-2569-0010` `customer_name=null` ส่วนใบบัตร `OR-2569-0009` มีชื่อ)
     → กระทบลูกค้า PromptPay ทุกคน · จุดเดียวกันที่ `:376` ก็ fallback เป็น `""` ได้เหมือนกัน
