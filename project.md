...
1b. **🔴 หัก ณ ที่จ่ายในเอกสารทุกใบ — ทำตอนจดบริษัท (ข้อ 0b)**
   · ~~ส่งตัวอย่างเอกสารใบสรุปการโอนส่วนแบ่ง~~ ✅ **เสร็จแล้ว (31 ก.ค. 2026)** — เอกสารทุกใบ
     (ใบเสนอราคา/ใบแจ้งหนี้/ใบเสร็จ/ใบสรุปโอนส่วนแบ่ง) ใช้เลย์เอาต์เดียวกันจาก `doc-layout.ts`
     พิกัดวัดจากไฟล์ต้นแบบ Illustrator ของเจ้าของ **ห้ามปรับระยะด้วยสายตา — วัดจากต้นแบบใหม่**
   · **ตอนนี้ใบสรุปโอนส่วนแบ่งยังไม่มีบรรทัดหักภาษี** เพราะยังจ่ายในนามบุคคล
     พอจดบริษัทแล้วต้องเพิ่ม และ**แก้ให้ครบทุกจุดที่เกี่ยวข้องพร้อมกัน** ไม่งั้นยอดจะไม่ตรงกัน
   ...
   · **เช็คลิสต์ตอนจดบริษัท — ต้องแก้ให้ครบทุกข้อ:**
     1. `src/lib/quote-doc.ts` — `WHT_RATE` (ค่าคงที่บนสุดของไฟล์) เป็นของ **ฝั่งลูกค้าองค์กร
        หักจาก designer** ไม่ใช่ของ payout · ย้ายไปเป็น helper ใน `src/lib/revenue.ts`
        (แบบเดียวกับ `designerShareOf()`) **แยกสองเคสให้ชัด ห้าม reuse อัตราเดียวกันอัตโนมัติ**
     2. `src/lib/payout-doc.ts` — เพิ่มแถวหักภาษีในตาราง (ระหว่างรายการกับแถบยอดโอนรวม
        ใช้ `drawTotalRow` แบบเดียวกับ `quote-doc.ts`) และเปลี่ยน "ยอดโอนรวม" เป็นยอดสุทธิ
     3. `PayoutDocData` เพิ่ม `whtRate`/`whtAmount`/`netAmount`/`docNo`
     4. เลขที่เอกสารใบสรุปโอน — ใช้ `next_doc_no()` ที่มีอยู่ (prefix ใหม่ เช่น `PO`)
        แล้วส่งเข้า `drawTitleRow` ทางขวาแบบใบเสนอราคา
     5. `admin/payouts` แสดงยอดก่อนหัก/ภาษีหัก/ยอดโอนจริง —
        **ช่องบันทึกจ่ายต้องเป็นยอดสุทธิ** ไม่งั้นยอดค้างไม่ตรง
     6. `/designer-agreement` เพิ่มข้อความว่าจ่ายหลังหักภาษี + ออกใบ 50 ทวิให้
     7. **ใบ 50 ทวิ** — เอกสารใบใหม่ ใช้ `doc-layout.ts` ชุดเดิมได้เลย (ขอต้นแบบจากเจ้าของก่อน)
     8. หัวเอกสารของแพลตฟอร์ม — แก้ `LEGAL_ENTITY*` ใน `src/lib/brand.ts` ที่เดียว
        (ชื่อ/เลขผู้เสียภาษี/ที่อยู่ของบริษัทใหม่) เอกสารทุกใบเปลี่ยนตามทันที
