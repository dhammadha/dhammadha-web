-- Admin insert/update font via SECURITY DEFINER RPC to bypass RLS
-- This avoids potential issues with get_my_role() being called inside WITH CHECK
...
declare
  ...
  v_result jsonb;
begin
  -- Verify caller is admin (get_my_role bypasses RLS via SECURITY DEFINER + set row_security=off)
