...
export default function DesignerPage() {
  ...
  useEffect(() => {
    if (!animated) {
      const id = requestAnimationFrame(() => { setAnimated(true); setPos((p) => {
  ...
  const handleTransitionEnd = () => {
    if (pos >= poolSize + showCount || pos < showCount) {
      setAnimated(false);
    }
  };
    ...
    <>
      ...
      <div className="bg-bg min-h-screen">
        <div className="max-w-site mx-auto px-8 py-10">
          ...
          {/* Featured slider — same logic as homepage selected cards */}
          {sliderPool.length > 0 && (
