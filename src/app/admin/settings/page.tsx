  ...
  return (
    <div className="p-6 max-w-[680px] flex flex-col gap-8">
      ...
      <Section title="โปรโมชั่นทั้งหมด" desc="เปิด/ปิดโปรโมชั่นสำหรับฟอนต์ทุกตัวที่ไม่ใช่ฟรีพร้อมกัน">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="ส่วนลด (%)"><input type="number" value={promoDiscount} onChange={(e) => setPromoDiscount(e.target.value)} placeholder="เช่น 30" min="0" max="100" className={iCls} /></Field>
          <Field label="วันสิ้นสุดโปรโมชั่น"><input type="date" value={promoEnd} onChange={(e) => setPromoEnd(e.target.value)} className={iCls} /></Field>
        </div>
