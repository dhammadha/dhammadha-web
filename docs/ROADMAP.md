...
## 0.3 Type tester (ทดสอบฟอนต์บนหน้าเว็บ)
...
**วิธีแก้:** ฟอนต์ขายใช้ **ไฟล์ demo** (bucket public) ในการ render tester
ส่วนฟอนต์ฟรีใช้ไฟล์ฟรีตามเดิม — ยืนยันแล้วว่าโหลดและแสดงผลจริง

**⚠️ ผลที่ตามมาเชิงปฏิบัติ:** ฟอนต์ขายที่**ไม่อัปโหลดไฟล์ demo จะไม่มี
type tester** และไม่แสดงรายชื่อ weight ให้เลือก — ตอน migrate ฟอนต์ 35 ตัว
ต้องใส่ demo ทุกตัว (จำนวน weight ที่โชว์บนหน้าเว็บใช้ `weight_count`
ที่นับจากไฟล์เต็มตอนบันทึก จึงยังแสดงถูกแม้ demo มีไม่ครบทุก weight)
...
## 1.2 ระบบ generate demo subset อัตโนมัติ

**ทำไมสำคัญ:** ตอนนี้ designer อัปไฟล์ demo เอง — เสี่ยงอัปไฟล์เต็มมาผิด
หรือทำ demo ที่ยัง copy ไปใช้งานจริงได้ พอเปิดรับ designer ภายนอก
เราคุมคุณภาพ/ความปลอดภัยของไฟล์ที่เขาอัปไม่ได้เลย

**แนวทาง:** pipeline ที่รับไฟล์เต็ม แล้ว generate demo ให้อัตโนมัติด้วย
fonttools/pyftsubset — ตัด glyph ให้เหลือบางส่วน (เช่น ก-ฮ สระหลัก A-Z 0-9)
+ เปลี่ยนชื่อฟอนต์ภายในไฟล์เป็น "ชื่อฟอนต์ DEMO" กันสับสน/กันแอบใช้
รันได้ 2 แบบ: script ฝั่ง local สำหรับแอดมิน (ง่ายสุด เริ่มก่อน) หรือ
Edge Function ที่ทำตอนอัปโหลด (อัตโนมัติเต็มรูปแบบ ทำทีหลังได้)
