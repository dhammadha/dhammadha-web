...
> อัปเดตล่าสุด: 8 ก.ค. 2026 — Phase 0 โค้ดเสร็จแล้ว รอทดสอบบน production
...
> **สถานะ (9 ก.ค. 2026 — commits c822888, f264687, d3a0883):** งานโค้ดส่วนใหญ่เสร็จ:
> pipeline ไฟล์ฟอนต์ (1.2), หน้า recruit `/become-a-designer` (1.3), ข้อตกลง designer
> `/designer-agreement` + ติ๊กยอมรับตอนสมัคร (1.4), login gate ฟอนต์ฟรี + PDPA consent (1.6),
> waitlist แทน ฿XXX (1.7), หน้า Legal ครบ (1.8), `_headers` + legacy redirect ผ่าน smart 404 +
> แก้ Nav search ที่เคยพาไป 404 (1.9 บางส่วน)
> **ยังเหลือ:** onboarding wizard + admin checklist (1.5), ย้ายฟอนต์ 35 ตัว (1.1),
> Zoho / DNS cutover (1.9) — AdSense เช็คแล้วเป็นของจริง ตัดออกได้ — และ **apply migration 0029**
...
## 1.2 Pipeline เตรียมไฟล์ฟอนต์ ✅ (`scripts/prepare_font_assets.py`)

รับไฟล์ฟอนต์เต็มทุก weight แล้วผลิต 2 อย่าง (ต้องการ `pip install fonttools brotli`):
...
วิธีใช้: `python3 scripts/prepare_font_assets.py --slug <slug> --family "<ชื่อ>"
--out ./font-assets <ไฟล์ทุก weight>` แล้วอัปโหลดผลลัพธ์ผ่าน FontForm
(ช่อง Tester Font + ไฟล์ map / ช่อง Demo Font)
ระยะยาว: ย้ายไปรันอัตโนมัติตอนอัปโหลด (Edge Function) เมื่อ designer ภายนอกเยอะขึ้น
...
## 1.5 Onboarding wizard + Quality checklist
...
- **Fingerprint ผู้ซื้อในไฟล์ฟอนต์** — ฝังชื่อ/เลข order ใน font metadata
  ตอน gen ไฟล์ให้ลูกค้า ไว้ trace ต้นทางถ้าไฟล์หลุดสู่สาธารณะ
