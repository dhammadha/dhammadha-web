...
**🔑 คู่สีของแบรนด์คือ `black #080808` + `page #F0F0F0` เท่านั้น** — ตรวจไฟล์
`public/brand/*.svg` ทั้ง 5 ตัวแล้ว **ลงสีจริงแค่สองค่านี้** (ส้มทั้งสามถูกประกาศไว้ใน `<style>`
ของ Illustrator แต่ไม่มี `path` ไหนใช้) → **ตัวอักษรบนพื้นดำใช้ `text-page` ไม่ใช่ `text-white`**
...
4. **บนพื้นดำห้ามใช้ `text-grey-600` (1.9:1)** — ข้อความรองบนดำใช้ **`grey-400`** (9.23:1 ✅)
5. **`text-white` ที่ยังเหลือ 45 จุดคือของที่อยู่บนพื้น feedback หรือทับรูป — ห้ามกวาดตาม**
   ขาวบน `success #0A8A84` ได้ 4.21:1 อยู่แล้ว ถ้าเปลี่ยนเป็น `page` จะเหลือ **3.70:1 = แย่ลง**
   คู่สองสีเป็นกติกาของ*แบรนด์* ไม่ใช่ของทุกพื้นในเว็บ

**สถานะ "เลือกอยู่" = พื้นดำ + อักษรเทา** (`text-page` / ข้อความรอง `text-grey-400`)
🔴 **ทุกจุดที่ทำแบบนี้ ลูกต้องพลิกสีตามพ่อครบทุกตัว** รวมป้ายที่ฝังอยู่ในแถว (`bg-black` → สลับเป็น
`bg-page`) ปุ่ม radio (`accent-black` → `accent-page`) และลิงก์ `.link-accent` (เติม `text-page` ทับ
— utility ปล่อยหลัง `@layer components` จึงชนะ) · **ไม่ทำ = ตัวดำบนพื้นดำ โดยไม่มี error**
**ยกเว้นบนพื้นมืดอยู่แล้ว** (sidebar admin/designer) → เลือก = **เม็ด `bg-page`** สลับคู่กัน
...
| `Badge` | `sale` เหลือง · `free`/`new` เทาพื้น-ดำตัว (ทับรูปปก) · `solid` ดำพื้น-เทาตัว (บทบาท/สถานะบนการ์ด) · `tag` เทาอ่อน |
...
(moodboard ไม่วาดแต่ตัดออก = เสียฟังก์ชัน) `sale`=warning · `free`/`new`=เทาพื้น-ดำตัว (ทับรูปปก จึงสลับคู่) · `solid`=ดำพื้น-เทาตัว
